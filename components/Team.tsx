        ...
        <div className='text-center'>
          ...
          <div className='mb-12'>
            <h2 className='text-4xl md:text-5xl font-light text-white mb-4'>
              Meet the team behind PickAxe
            </h2>
            <p className='text-white/80 text-sm'>
              A passionate team dedicated to connecting skilled workers with those who need their expertise.
            </p>
          </div>
            ...
            {teamMembers.map((member, index) => (
              <div key={index} className='text-center'>
                ...
                <div className='flex items-center justify-center space-x-1'>
                  ...
                  {member.hasLinkedIn && <Linkedin className='w-4 h-4 text-white' />}
                </div>
                {member.role && <p className='text-white/70 text-xs mt-1'>{member.role}</p>}
              </div>
            ))}
          </div>
        </div>
